...
class GarbAdapter : ListAdapter<GarbModel, GarbAdapter.ItemHolder>(ItemComparator()) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemHolder {
        val layoutItemGarb = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_garb, parent, false)
        return ItemHolder(layoutItemGarb)
    }
    ...
    class ItemHolder(view: View): RecyclerView.ViewHolder(view) {
       ...
       // @SuppressLint("SetTextI18n")
        fun setData(garbModel: GarbModel) = with(binding) {
            tvGarb.text = garbModel.garbName
        }
    }
